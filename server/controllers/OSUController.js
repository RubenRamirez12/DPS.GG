...
export const getBeatmapScores = async (req, res) => {
...
export const getUserBest = async (req, res) => {
  ...
  try {
    const userBestScores = await OsuClient.getUserBest(
      osuUsername,
      osuGameMode
    );
...
export const getUserRecent = async (req, res) => {

  try {
    const osuUsername = req.params.osuUsername
    const userRecentScores = await OsuClient.getUserRecent(osuUsername)
    res.status(200).json(userRecentScores)
  } catch (error) {
    res.status(404).json({ message: error.message })
  }
}
